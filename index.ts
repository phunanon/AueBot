...
bot.on('messageCreate', async (message: Message) => {
  ...
  let mentionedVerses = [
    ...content
      .replaceAll(/\s/g, '')
      .matchAll(/(?:_|\*)(a?b?c?d?e?f?g?h?i?j?k?l?m?n?o?)(?:_|\*)/g),
  ].flatMap(m => m[1].split(''));
  mentionedVerses = [...new Set(mentionedVerses)].sort();
  const mentionedVersesVN = mentionedVerses.map(
    m => [m, m.charCodeAt(0) - 97] as [string, number]
  );
